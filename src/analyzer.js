...
const ERROR_CATEGORIZATION_RULES = [
    ...
    details: [
      {
        ...
        detailMessage: "Variable/Function not defined",
      },
  ...
  },
  {
    ...
    details: [
      {
        regex: /Timed out after waiting `\d+ms` for your remote page to load/,
        detailMessage: "Page Load Timeout in Before Each",
      },
  ...
  // High priority errors
  {
    ...
    details: [
      {
        regex:
          /Timed out retrying after \d+ms: \[object Object\]: expected 'https:\/\/auth\.microtecstage\.com\/Account\/Login\?returnUrl=/,
        detailMessage: "Login URL Mismatch",
      },
      {
        regex: /expected '' to match/,
        detailMessage: "Empty String Match Failure",
      },
      {
        regex: /expected \d+ to equal \d+/,
        detailMessage: "Numeric Equality Assertion Failed",
      },
      {
        regex: /Expected to find element: `([^`]+)`, but never found it\./,
        detailMessage: "Element Not Found (Selector: $1)",
      }, // Capture selector
      {
        regex:
          /Expected to find content: '([^']+)' within the selector: '([^']+)' but never did\./,
        detailMessage: "Content Not Found (Content: $1, Selector: $2)",
      }, // Capture content and selector
      {
        regex: /expected '<label\.form-label\.paragraph_b18>' to be 'visible'/,
        detailMessage: "Label Visibility Assertion Failed",
      },
      {
        regex: /Timed out retrying after/,
        detailMessage: "Generic Assertion Timeout",
      },
  ...
  // Medium priority errors
  {
    ...
    details: [
      {
        regex: /`cy\.visit\(\)` failed trying to load/,
        detailMessage: "Cypress Visit Load Failure",
      },
      {
        regex:
          /`cy\.click\(\)` failed because the page updated while this command was executing/,
        detailMessage: "Cypress Click Stale Element",
      },
      {
        regex: /`cy\.click\(\)` failed because this element/,
        detailMessage: "Cypress Click Covered Element",
      },
      {
        regex:
          /`cy\.wait\(\)` timed out waiting `\d+ms` for the(?: \d+(?:st|nd|rd|th))? request to the route: `([^`]+)`\. No request ever occurred\./,
        detailMessage: "Cypress Wait Request Timeout (Route: $1)",
      }, // Capture route
      {
        regex: /`match` requires its argument be a `RegExp`/,
        detailMessage: "Cypress Match Argument Invalid",
      },
      {
        regex:
          /`cy\.scrollIntoView\(\)` can only be used to scroll to \d+ element, you tried to scroll to \d+ elements\./,
        detailMessage: "Cypress Scroll Multi-Element",
      },
      {
        regex: /is being covered by another element/,
        detailMessage: "Element Covered by Another",
      },
      {
        regex: /No request ever occurred\./,
        detailMessage: "Cypress Request Never Occurred",
      }, // Generic request wait timeout
  ...
  // Default / Low priority
  {
    ...
    priority: "Low",
    details: [{ regex: /.*/, detailMessage: "Unclassified Error Detail" }],
  },
];
...
function getErrorCategoryAndPriority(errorMessage) {
  ...
  return {
    ...
    priority: "Unknown",
    detail: "No matching category or detail found",
  };
}
  ...
  try {
    ...
    const processTest = (suitePath, suiteTitle, test) => {
      if (test.fail) {
        ...
        const { category, priority, detail } =
          getErrorCategoryAndPriority(errorMessage);
    ...
    const processSuites = (suitePath, suites) => {
      suites.forEach((suite) => {
        ...
        suite.tests.forEach((test) =>
          processTest(currentSuitePath, currentSuiteTitle, test)
        );
    ...
    if (!rawData || !Array.isArray(rawData.results)) {
      console.warn(
        "Input JSON does not contain a valid 'results' array. No tests to process."
      );
    ...
    rawData.results.forEach((specFileResult) => {
      ...
      specFileResult.tests.forEach((test) =>
        processTest(specFilePath, specFileResult.title || "Root Suite", test)
      );
